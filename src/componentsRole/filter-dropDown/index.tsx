...
export default function FilterDropdown({ open, onClose, onSelect, anchorRef }: FilterDropdownProps) {
  ...
  return (
    ...
    >
      <div className="dropdown-panel">
        ...
        <section className="dropdown-section" aria-label="Filter By Role">
          <header className="dropdown-label">Filter By Role</header>
          <ul className="dropdown-list">
            {roleOptions.map((option) => (
              <li
                ...
                aria-checked="false"
                tabIndex={open ? 0 : -1}
                onClick={() => handleSelect('role', option.value)}
              >
                {option.label}
              </li>
            ))}
